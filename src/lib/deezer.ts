...
// S-boxes are large — load from a generated constant
function generateSBoxes(): Uint32Array[] {
  // Standard Blowfish S-box initial values (4 × 256 entries)
  // These are the fractional parts of pi
  const raw = [
    // S-box 0
  ...
  return raw.map((box) => new Uint32Array(box));
}
...
class Blowfish {
  ...
  constructor(key: Buffer) {
    this.P = new Uint32Array(BF_P_INIT);
    this.S = generateSBoxes();
    this.expandKey(key);
  }
...
function blowfishDecryptCBC(data: Buffer, key: Buffer, iv: Buffer): Buffer {
  ...
  for (let i = 0; i < data.length; i += 8) {
    const encL = data.readUInt32BE(i);
    const encR = data.readUInt32BE(i + 4);
    let [decL, decR] = bf.decryptBlock(encL, encR);
    decL = (decL ^ prevL) >>> 0;
    decR = (decR ^ prevR) >>> 0;
    out.writeUInt32BE(decL, i);
    out.writeUInt32BE(decR, i + 4);
    prevL = encL;
    prevR = encR;
  }
  return out;
...
function decryptChunk(chunk: Buffer, blowfishKey: Buffer): Buffer {
  ...
  return blowfishDecryptCBC(chunk, blowfishKey, iv);
}
